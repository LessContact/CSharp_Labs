...
namespace T7.TableService.Models;

public class Fork {
    public int Id { get; }
    public ForkState State { get; private set; } = ForkState.Available;
    public string? HeldByPhilosopher { get; private set; }
    public string? HeldByPhilosopherName { get; private set; }
    ...
    public bool TryTake(string philosopherId, string philosopherName) {
        lock (_lock) {
            ...
            HeldByPhilosopher = philosopherId;
            HeldByPhilosopherName = philosopherName;
            _isEating = false;
    ...
    public bool Release(string philosopherId) {
        lock (_lock) {
            if (HeldByPhilosopher != philosopherId) return false;

            RecordElapsedTime();

            State = ForkState.Available;
            HeldByPhilosopher = null;
            HeldByPhilosopherName = null;
            _isEating = false;
            _stateStartTime = DateTime.UtcNow;
    ...
    public ForkInfo ToForkInfo() {
        lock (_lock) {
            return new ForkInfo {
                Id = Id,
                State = State,
                HeldByPhilosopher = HeldByPhilosopherName
            };
        }
    }
}
